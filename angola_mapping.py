...
def get_rc_loci_dic (loci_dic):
    ...
    loci_list_rc = {"ga": None, "gb": None, "cc": None}

    for loci, values in loci_dic.items():
        for k,v in alt_map.iteritems():
            ga_rc = str(values["ga"]).replace(k,v)
            bases = list(ga_rc)
            bases = reversed([complement.get(base,base) for base in bases])
            bases = "".join(bases)
            for k,v in alt_map.iteritems():
                bases = bases.replace(v,k)
            loci_list_rc["ga"] = bases
        for k,v in alt_map.iteritems():
            gb_rc = str(values["gb"]).replace(k,v)
            bases = list(gb_rc)
            bases = reversed([complement.get(base,base) for base in bases])
            bases = "".join(bases)
            for k,v in alt_map.iteritems():
                bases = bases.replace(v,k)
            loci_list_rc["gb"] = bases

        for k,v in alt_map.iteritems():
            cc_rc = str(values["cc"]).replace(k,v)
            bases = list(cc_rc)
            bases = reversed([complement.get(base,base) for base in bases])
            bases = "".join(bases)
            for k,v in alt_map.iteritems():
                bases = bases.replace(v,k)
            loci_list_rc["cc"] = bases
        loci_dic_rc[loci] = loci_list_rc
        loci_list_rc = {"gb": None, "ga": None, "cc": None}
...
def mutation_type(sam_dic, gff3_dic, loci_dic, loci_dic_rc):
    ...
    #list_clone_a = ["317", "1402", "1516", "4730", "4730", "5581", "8664", "10475", "10841", "12389", "12593", "13233", "13233", "13233", "13233", "13701", "14462", "14850", "15487", "15487", "19398", "19464", "21377", "23077", "23236", "23717", "24902", "24925", "25213", "29172", "35009", "36814", "37640", "37724", "38152", "41340", "41839", "42379", "42379", "42379", "42379", "42379", "44040", "44569", "45798", "46722", "46765", "47375", "47780", "48339", "48814", "49865", "50738"]    
    #list_clone_b = ["1245", "4992", "6834", "7821", "7829", "10064", "14810", "15800", "15800", "15800", "15800", "17841", "18992", "23725", "26273", "26873", "28038", "29557", "30985", "30999", "32954", "33466", "34514", "35538", "37369", "39387", "47930", "50050"]
#    new_list = []

 #   for a, b in sam_dic.items ():

       
  #      if sam_dic[a]["element_type"] != ['intergenic']:
 
   #         new_list.append(a)


    #new_list=((set(new_list)).difference(set(list_clone_a).symmetric_difference(list_clone_b)))
   # for a, b in sam_dic.items ():
    #    for e in new_list:
     #       if a == e:
      #          print(a, sam_dic[a]["element_type"], sam_dic[a]["aa_information"], sam_dic[a]["scaffold"], sam_dic[a]["real_snp_localization"], sam_dic[a]["start"], sam_dic[a]["snp_variation"], sam_dic[a]["description"])

   # print (sam_dic)
   #      for e in list_clone_a:
   #          if e == a:
   #              print("Locus_clone_b")
   #              print(sam_dic[a]["gene_index"],a, sam_dic[a]["aa_information"], sam_dic[a]["scaffold"], sam_dic[a]["real_snp_localization"])

        #     else:
        #         pass

    #print(sam_dic[locus]["gene_index"],locus, sam_dic[locus]["description"], sam_dic[locus]["aa_information"], sam_dic[locus]["scaffold112"], sam_dic[locus]["real_snp_localization"])
...
def find_real_codon (loci_value, codon_pos, snp_position, locus):
    real_codon = {"ga": None, "gb": None, "cc": None}

    real_codon_final = {}
    count_error = 0
    if loci_value["gb"] == None or loci_value["ga"] == None or loci_value["cc"] == None:
        count_error += 1
        pass
        ...
        if codon_pos == 0:
            real_codon["ga"] = loci_value["ga"][snp_position-3:snp_position]
            real_codon["gb"] = loci_value["gb"][snp_position-3:snp_position]
            real_codon["cc"] = loci_value["cc"][snp_position-3:snp_position]

        elif codon_pos == 2:
            real_codon["ga"] = loci_value["ga"][snp_position-2: snp_position+1]
            real_codon["gb"] = loci_value["gb"][snp_position-2: snp_position+1]
            real_codon["cc"] = loci_value["cc"][snp_position-2: snp_position+1]

        elif codon_pos == 1:
            real_codon["ga"] = loci_value["ga"][snp_position-1:snp_position+2]
            real_codon["gb"] = loci_value["gb"][snp_position-1:snp_position+2]
            real_codon["cc"] = loci_value["cc"][snp_position-1:snp_position+2]
...
def translate_dna(real_codon, locus):
    ...
    if len(str(real_codon[locus]["ga"])) < 3:
        bad_locus_list = locus


    else:
        aa_dic["aa_ga"] = codontable[real_codon[locus]["ga"]]
        aa_dic["aa_gb"] = codontable[real_codon[locus]["gb"]]
        aa_dic["aa_cc"] = codontable[real_codon[locus]["cc"]]
        if len(set([aa_dic["aa_cc"], aa_dic["aa_ga"], aa_dic["aa_gb"]])) == 1:
            count_syn += 1
            mutation = "synonymous"
        elif len(set([aa_dic["aa_cc"], aa_dic["aa_ga"], aa_dic["aa_gb"]])) and (aa_dic["aa_ga"] != aa_dic["aa_cc"] or aa_dic["aa_ga"] != aa_dic["aa_gb"]):
            count_nsyn += 1
